...
subprocess.call("head -" + str(
    int(math.floor(line_count / 2))) + " cisbemon.txt | awk 'length() == 508' > " + output_dir + "/3_lp.txt",
                shell=True)
...
subprocess.call("tail -" + str(
    int(math.floor(line_count / 2))) + " cisbemon.txt |awk 'length() == 508' > " + output_dir + "/9_llc.txt",
                shell=True)
...
for file in glob.glob("*.csv"):
    subprocess.call(
        "in2csv -e us-ascii-f fixed -s " + file + " ../output/" + file.replace(".csv", ".txt") + " > ../output/" + file,
        shell=True)
